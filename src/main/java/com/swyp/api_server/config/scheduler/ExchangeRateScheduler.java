...
public class ExchangeRateScheduler {
    ...
    /**
     * 매일 오전 9시 10분에 환율 데이터 갱신 및 히스토리 저장
     * - 한국수출입은행 API 호출 후 최신 환율 업데이트
     * - 현재 환율을 히스토리 테이블에 저장 (변동률 계산용)
     * 
     * 크론 표현식: "초 분 시 일 월 요일"
     * 0 10 9 * * MON-FRI : 월~금 오전 9시 10분
     */
    @Scheduled(cron = "0 10 9 * * MON-FRI", zone = "Asia/Seoul")
    public void updateAndSaveExchangeRates() {
        log.info("========== 환율 데이터 갱신 및 히스토리 저장 시작 ==========");
        
        try {
            // 1. 최신 환율 데이터 갱신 (한국수출입은행 API 호출)
            log.info("1. 최신 환율 데이터 갱신 중...");
            List<ExchangeResponseDTO> latestRates = exchangeRateService.getAllExchangeRates();
            log.info("✓ 환율 데이터 갱신 완료: {}개 통화", latestRates.size());

            // 2. 환율 데이터를 DB에 저장
            log.info("2. 환율 데이터 DB 저장 중...");
            storageService.saveExchangeRates(latestRates, "KOREA_EXIM");
            log.info("✓ 환율 데이터 DB 저장 완료");

            // 3. 현재 환율을 히스토리 테이블에 저장
            log.info("3. 환율 히스토리 저장 중...");
            historyService.saveCurrentRatesAsHistory();
            log.info("✓ 환율 히스토리 저장 완료");

            log.info("========== 환율 데이터 갱신 및 히스토리 저장 완료 ==========");
            
        } catch (Exception e) {
            log.error("환율 데이터 갱신 중 오류 발생", e);
            // 여기에 알림 시스템 연동 가능 (슬랙, 이메일 등)
        }
    }

    /**
     * 매시간 정각에 환율 데이터 갱신 (실시간성 확보)
